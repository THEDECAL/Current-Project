...
namespace OnlinePoker.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _emailSender.SendEmailAsync(Input.Email, "Подтверждение электронной почты",
                        $"Пожалуйста подтвердите вашу почту <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>перейдя по этой ссылке</a>.");
