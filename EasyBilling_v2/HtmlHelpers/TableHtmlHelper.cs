...
{
    public class TableHtmlHelper<TEntity> where TEntity : class
    {
        const int PAGE_COUNT = 3; //Кол-во страниц на секцию
        private Type _type;
        private TableDataViewModel<TEntity> _data;

        public TableHtmlHelper(TableDataViewModel<TEntity> data)
        {
            ...
            return await Task.Run(async () =>
            {
                ...
                foreach (var item in props)
                {
                        ...
                        noShowAtt == null)
                    {
                        ...
                        if (_data.State.SortField.Equals(item.Name))
                        {
                            aFieldLink.InnerHtml.AppendFormat($"<span class='m-1 badge badge-info'>{(_data.State.IsOrderByDesc ? "˄" : "˅")}</span>");
                        }
        ...
        public async Task<HtmlString> GetPaginationPanelAsync()
        {
            return await Task.Run(async () =>
            {
                ...
                else
                {
                    ...
                    var valPrev = (_data.State.CurrentPage - 1).ToString();
                ...
                else
                {
                    ...
                    var valNext = (_data.State.CurrentPage + 1).ToString();
                ...
                int currSection = (_data.State.CurrentPage - 1) / PAGE_COUNT; //Текущая секция страниц (с ноля)
                for (int i = 1; i <= PAGE_COUNT; i++)
                {
                    ...
                    if (pageNum <= _data.AmountPage) //Если текущая страница входит в общее кол-во страниц
                    {
                        ...
                        li.Attributes.Add("class", $"page-item {(_data.State.CurrentPage == pageNum ? "active" : "")}");
                        ul.InnerHtml.AppendHtml(li);

                        if (_data.State.CurrentPage == pageNum)
                        {
                            TagBuilder span = new TagBuilder("span");
                            span.Attributes.Add("class", "page-link");
        ...
        public async Task<HtmlString> GetControlPanelAsync(bool isShowBtnAdd = true)
        {
            return await Task.Run(async () =>
            {
                ...
                input.AddCssClass("form-control");
                input.Attributes.Add("type", "text");
                input.Attributes.Add("id", nameof(PaginationState.SearchText));
                input.Attributes.Add("placeholder", "Поиск по всем полям");
                input.Attributes.Add("style", "width: 500px");
                input.Attributes.Add("value", _data.State.SearchText);
                ...
                div.InnerHtml.AppendFormat($"<input type='hidden' id='{nameof(PaginationState.RowsCountPerPage)}' value='{_data.State.RowsCountPerPage}' />");
                div.InnerHtml.AppendFormat($"<input type='hidden' id='{nameof(PaginationState.IsOrderByDesc)}' value='{(_data.State.IsOrderByDesc ? 1 : 0)}' />");
