...
namespace EasyBilling.Services
{
    public class AccessRightsManager
    {
        ...
        private async Task<User> GetAccountAsync([NotNull] string userName)
        {
        ...
        private async Task<IList<string>> GetRolesAsync([NotNull] User acc)
        ...
        private async Task<IList<string>> GetRolesAsync([NotNull] string userName)
        {
            if (!string.IsNullOrWhiteSpace(userName))
            {
                User acc = await GetAccountAsync(userName);
                if (acc != null)
                {
        ...
        public async Task<Role> GetRoleAsync(string userName)
        {
            ...
            return await _dbContext.Set<Role>()
                ...
                .FirstOrDefaultAsync(r => r.Name.Equals(roles.FirstOrDefault()));
        }
