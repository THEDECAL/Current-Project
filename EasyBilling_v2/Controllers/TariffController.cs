...
{
    [DisplayName("Тарифы")]
    public class TariffController : ExtController<Tariff>
    {
        public TariffController(BillingDbContext dbContext, RoleManager<Role> roleManager, UserManager<User> userManager, IServiceScopeFactory scopeFactory) : base(dbContext, roleManager, userManager, scopeFactory)
        {
        }
        ...
        //    {
        //        var dvm = new TableDataViewModel<Tariff>(_scopeFactory,
        ...
        public async Task<IActionResult> AddUpdateForm(int? id = null)
        {
            return await Task.Run(async () =>
            {
                ...
                if (id != null)
                {
                    model = await DbContext.Tariffs.FindAsync(id);
                    if (model == null)
                        model = new Tariff();
                    else
        ...
        public async Task<IActionResult> Create(Tariff obj)
        {
            ...
            {
                await DbContext.Tariffs.AddAsync(obj);
        ...
        public async Task<IActionResult> Update(Tariff obj)
        {
            ...
            if (ModelState.IsValid)
            {
                var objExisting = await DbContext.Tariffs
        ...
        {
            var obj = await DbContext.Tariffs.FindAsync(id);
            await Task.Run(() =>
            {
                if (obj != null)
                {
                    DbContext.Tariffs.Remove(obj);
                    DbContext.SaveChanges();
                }
            });
        ...
        public async Task ServerSideValidation(Tariff obj)
        {
            ...
            {
                var tariffExisting = await DbContext.Tariffs
                    .FirstOrDefaultAsync(t => t.Id.Equals(obj.Id));
