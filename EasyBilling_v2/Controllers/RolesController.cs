...
{
    [DisplayName("Роли")]
    public class RolesController : ExtController<ExtEntity>
    {
        public RolesController(BillingDbContext dbContext, RoleManager<Role> roleManager, UserManager<User> userManager, IServiceScopeFactory scopeFactory)
        ...
        //    {
        //        var dvm = new TableDataViewModel<Role>(_scopeFactory,
        //            settings: Settings,
        ...
        public async Task<IActionResult> AddUpdateForm(string id = null)
        {
            return await Task.Run(() =>
            {
                ...
                if (id != null)
                {
                    model = DbContext.Set<Role>()
                        .Include(r => r.DefaultControllerName)
        ...
        public async Task<IActionResult> Update(Role obj)
        {
            ...
            if (ModelState.IsValid)
            {
                var defaultControllerName = await DbContext.ControllersNames
                    .FirstOrDefaultAsync(c => c.Name.Equals(obj.DefaultControllerName.Name));
                obj.DefaultControllerNameId = defaultControllerName.Id;

                var objExisting = await DbContext.Set<Role>()
                    .FirstOrDefaultAsync(o => o.Id.Equals(obj.Id));

                await Task.Run(() =>
                {
                    DbContext.Entry(objExisting).CurrentValues.SetValues(obj);
        ...
        {
            var obj = await DbContext.Roles.FindAsync(id);
            await Task.Run(() =>
            {
                if (obj != null)
                {
                    if (!DbContext.UserRoles.Any(ur => ur.RoleId.Equals(id)))
                    {
        ...
        public async Task ServerSideValidation(Role obj)
        {
            ...
            var isControllerExist = await DbContext
            ...
            var isRoleExist = await DbContext
